...
var mappingStructure = {
    ...
    keyboardHook: {
        def: ' full-screen',
        name: 'keyboardhook:i:',
        fn: arrayMatchTransform(['local', 'remote', ' full-screen'], 0)
    },
    displayConnectionBar: {
        def: true,
        name: 'displayconnectionbar:i:',
        fn: arrayMatchTransform([false, true])
    },
    bitmapCachePersistEnable: {
        def: true,
        name: 'bitmapcachepersistenable:i:',
        fn: arrayMatchTransform([false, true])
...
function buildRdpFile(config) {
	...
	let promise = new Promise((resolve, reject) => {
		...
		fs.writeFile(fileName, fileContent, {encoding: 'windows1251'}, (err) => {
		  if (err) reject(err);
		  resolve(fileName);
		});
	});
